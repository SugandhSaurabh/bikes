...
class clscon
{
    ...
    public function db_connect()
    {
     ...
     $username="root";
     $pwd="";
     $this->link=  mysqli_connect($host, $username, $pwd,"dbrentandride") or die("connection error".mysqli_error($this->link));
     return $this->link;
    }
...

    {
        if($_SESSION["cart"]!=" ")
        {
            ...
            {
                $qry="select * from avail_bikes where bike_id=$id";
                $res=mysqli_query($link,$qry);
                if($r=mysqli_fetch_row($res))
                {
                    $output[]='<tr>';
                    $output[]='<td>';
                    $output[]="<a href=cart.php?bid=$r[6]&action=Delete>Delete</a>";
                    $output[]="</td>";
                    $output[]="<td>";
                    $output[]="$r[1]";
                    $output[]="</td>";
